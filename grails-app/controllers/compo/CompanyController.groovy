...
class CompanyController {
    ...
    def delete(Company companyInstance) {
        ...
        if (companyInstance.isDelateAble()) {
            ...
            request.withFormat {
                form multipartForm {
                    flash.message = message(code: 'default.not.deleted.message', args: [message(code: 'company.label', default: 'Company'), companyInstance.id])
                    redirect action: "show", method: "GET", id: companyInstance.id
                }
                '*' { render status: NO_CONTENT }
