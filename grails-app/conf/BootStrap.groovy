...
class BootStrap {
    ...
    def init = { servletContext ->
        ...
        c1.addToPersons(p1)
        c1.addToPersons(p2)
        c1.deleteStatus = CheckDeleteable.DeleteableStatus.DELETEABLE
        c1.save(flush: true)

        Company c2 = new Company(name: 'Company 2 NOT_DELETE_ABLE')
        c2.addToPersons(p1)
        c2.addToPersons(p2)
        c2.deleteStatus = CheckDeleteable.DeleteableStatus.NOTDELETEABLE
        c2.save(flush: true)

        Company c3 = new Company(name: 'Company 3 DELETE_ABLE')
        c3.addToPersons(p1)
        c3.addToPersons(p2)
        c3.deleteStatus = CheckDeleteable.DeleteableStatus.DELETEABLE
        c3.save(flush: true)

        Company c4 = new Company(name: 'Company 4 NOT_DELETE_ABLE')
        c4.addToPersons(p1)
        c4.addToPersons(p2)
        c4.deleteStatus = CheckDeleteable.DeleteableStatus.NOTDELETEABLE
        c4.save(flush: true)
